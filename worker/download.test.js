  ...
  const f = await fixture(t, {
    async run(command, args) {
      ...
      if (args.includes("--dump-single-json")) {
        ...
        return { stdout: JSON.stringify({ title: "Video", duration: 20, height: 720, availability: "public" }) };
      }
      assert.ok(args.includes("--load-info-json"));
      assert.equal(args[args.indexOf("-f") + 1], formatSelector("720p"));
      await writeFile(args[args.indexOf("-o") + 1].replace("%(ext)s", "mp4"), "media");
      return { stdout: "" };
    },
    async upload() { return { secure_url: "https://res.cloudinary.com/demo/video/upload/v1/test.mp4" }; }
  });
  const started = await f.start();
  const job = await f.finish(started.jobId);
  assert.equal(job.status, "completed");
  assert.equal(job.attempt, 2);
  assert.match(job.downloadUrl, /\/fl_attachment\//);
